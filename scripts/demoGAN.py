...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        # 计算真实图片的损失
        real_scores = D(real_img)  # 将真实图片放入判别器中
        # 计算假的图片的损失
        z = Variable(torch.randn(num_img, z_dimension)).cuda()  # 随机生成一些噪声
        fake_scores = D(G(z))  # 随机噪声放入生成网络中，生成一张假的图片，判别器判断假的图片
        # 损失函数和优化
        d_loss = criterion(real_scores, real_label) + criterion(fake_scores, fake_label)
        ...
        output = D(G(z))  # 随机噪声输入到生成器中，得到一副假的图片，经过判别器得到的结果
